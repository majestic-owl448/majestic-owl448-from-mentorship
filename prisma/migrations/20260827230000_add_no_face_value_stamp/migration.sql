...
-- RedefineTable
CREATE TABLE "new_stamp_inventory_entries" (
    ...
    CONSTRAINT "stamp_inventory_entries_named_face_value_id_country_code_fkey" FOREIGN KEY ("named_face_value_id", "country_code") REFERENCES "named_face_values" ("id", "country_code") ON DELETE RESTRICT ON UPDATE CASCADE,
    ...
    CONSTRAINT "stamp_inventory_entries_face_value_check" CHECK (
        ("face_value_type" = 'MONETARY' AND "face_amount" IS NOT NULL AND "face_currency_code" IS NOT NULL AND "named_face_value_id" IS NULL)
        OR
        ("face_value_type" = 'NAMED' AND "face_amount" IS NULL AND "face_currency_code" IS NULL AND "named_face_value_id" IS NOT NULL)
        OR
        ("face_value_type" = 'NONE' AND "face_amount" IS NULL AND "face_currency_code" IS NULL AND "named_face_value_id" IS NULL AND "manual_postage_amount" IS NOT NULL AND "manual_postage_currency_code" IS NOT NULL)
    )
);

INSERT INTO "new_stamp_inventory_entries" (
    "country_code", "created_at", "expired", "face_amount",
    "face_currency_code", "face_value_type", "id", "manual_postage_amount",
    "manual_postage_currency_code", "name", "named_face_value_id",
    "postal_entity_id", "quantity_annulled", "quantity_owned", "updated_at",
    "user_id", "year_of_issue"
)
SELECT
    "country_code", "created_at", "expired", "face_amount",
    "face_currency_code", "face_value_type", "id", "manual_postage_amount",
...
ALTER TABLE "new_stamp_inventory_entries" RENAME TO "stamp_inventory_entries";
